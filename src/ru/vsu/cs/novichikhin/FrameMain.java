...
public class FrameMain extends JFrame {
    ...
    public FrameMain() {
        ...
        this.setBounds(250, 50, 1500, 750);
        this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        this.pack();

        JTableUtils.initJTableForArray(tableInput, 230, false, true, true, false, true);
        JTableUtils.initJTableForArray(tableOutput, 550, false, true, true, false, false);
        ...
        buttonLoadFromInputFile.addActionListener(actionEvent -> {
            try {
                if (fileChooserOpen.showOpenDialog(panelMain) == JFileChooser.APPROVE_OPTION) {
                    List<List<String>> students = ListUtils.readList2FromFile(fileChooserOpen.getSelectedFile().getPath());
                    assert students != null;

                    String[][] array = ListUtils.toStringArray2(students);
        ...
        buttonSaveToInputFile.addActionListener(actionEvent -> {
            try {
                if (fileChooserSave.showSaveDialog(panelMain) == JFileChooser.APPROVE_OPTION) {
                    ...
                    ListUtils.writeList2ToFile(students, file);
                }
            } catch (Exception e) {
        ...
        buttonMakePairs.addActionListener(actionEvent -> {
            try {
                ...
                String[][] matrix = ListUtils.toStringArray2(pairs);
        ...
        buttonSaveToOutputFile.addActionListener(actionEvent -> {
            try {
                if (fileChooserSave.showSaveDialog(panelMain) == JFileChooser.APPROVE_OPTION) {
                    ...
                    assert array != null;
                    List<List<String>> pairs = ListUtils.toList2(array);
                    String file = fileChooserSave.getSelectedFile().getPath();
                    if (!file.toLowerCase().endsWith(".txt")) {
                        file += ".txt";
                    }
                    ListUtils.writeList2ToFile(pairs, file);
                }
            } catch (Exception e) {
                SwingUtils.showInfoMessageBox("Не уадётся сохранить в файл", "Ошибка");
            }
