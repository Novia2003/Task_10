...
public class ConsoleMain {
    ...
    private static void makePairs(ConsoleMain.CmdParams params) throws FileNotFoundException {
        try {
            List<List<String>> students = MyUtils.readList2FromFile(params.inputFile);
    ...
    private static void printResult(ConsoleMain.CmdParams params, List<List<String>> list) throws FileNotFoundException {
        if (params.outputFile != null) {
            MyUtils.writeList2ToFile(list, params.outputFile);
        } else {
            MyUtils.writeList2ToConsole(list);
        }
    }

    private static void printError() {
        System.err.print("Данные в файле некоректны (в каждой строке через запятую должны быть указаны ФИО, " +
