...
public class JTableUtils {
    ...
    public static void initJTableForArray(
        ...
        int changeButtonsSize, int changeButtonsMargin, boolean isInputTable
    ) {
        ...
        DefaultTableModel tableModel = new DefaultTableModel(new String[] { "[0]" }, 1) {
            ...
            public String getColumnName(int index) {
                List<String> list ;
                if (isInputTable) {
                    list = Arrays.asList("ФИО", "пол", "№ курса", "средний балл");
                } else {
                    list = Arrays.asList("№ курса", "Пара");
                }
                return String.format("%s", list.get(index));
            }
        };
    ...
    /**
     * Аналогичен {@link #initJTableForArray(javax.swing.JTable, int, boolean, boolean, boolean, boolean, int, int,, boolean) }.
     ...
     *
     * @see #initJTableForArray(javax.swing.JTable, int, boolean, boolean, boolean, boolean,  int, int, boolean)
     */
    public static void initJTableForArray(
        JTable table, int defaultColWidth,
        boolean showRowsIndexes, boolean showColsIndexes,
        boolean changeRowsCountButtons, boolean changeColsCountButtons, boolean isInputTable
    ) {
        initJTableForArray(
            table, defaultColWidth,
            showRowsIndexes, showColsIndexes, changeRowsCountButtons, changeColsCountButtons,
            22, DEFAULT_GAP, isInputTable
        );
    }
     ...
     * @param array
     */
    public static void writeArrayToJTable(JScrollPane table, String[][] array) {
        writeArrayToJTable(table, array);
    }
